...
class TestDeterministicTransientFailureMetrics:
    ...
    def test_all_nodes_transient_failure_is_not_data_loss(self):
        """All 3 nodes fail transiently at the same time — should NOT be data loss.

        Transient failures mean nodes are unavailable but still have their data.
        Even if every node is simultaneously down, data is not lost because
        each node retains its state and can recover.

        Timeline  (3 nodes, quorum = 2)
        --------
        0–1 h : 3 nodes up-to-date       →  can_commit = True
        1–2 h : all 3 nodes down          →  can_commit = False  (NOT data loss)
        2–3 h : all 3 nodes recovered     →  can_commit = True

        Availability = 2 h / 3 h = 2/3
        """
        # All 3 nodes: fail at 1 h, recover 1 h later
        fragile_config = NodeConfig(
            region="us-east",
            cost_per_hour=1.0,
        ...
        nodes = {
            "node0": NodeState(node_id="node0", config=fragile_config),
            "node1": NodeState(node_id="node1", config=fragile_config),
            "node2": NodeState(node_id="node2", config=fragile_config),
        }
        cluster = ClusterState(
            nodes=nodes,
            network=NetworkState(),
        ...
        result = simulator.run_for(hours(3))

        # KEY ASSERTION: transient failure is NOT data loss
        assert result.end_reason == "time_limit", (
            f"Expected 'time_limit' but got '{result.end_reason}'. "
            "Simultaneous transient failures should NOT count as data loss."
        )

        # All 3 nodes failed at t=1h (and may fail again at t=3h boundary)
        failure_events = [
            e for e in result.event_log
            if e.event_type == EventType.NODE_FAILURE
        ]
        ...
        # Availability = 2/3 (1 h down out of 3 h)
        assert result.metrics.availability_fraction() == pytest.approx(2 / 3)
